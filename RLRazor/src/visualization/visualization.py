...
def plot_pareto_frontier(results, dataset_name="math", use_curve_fit=True, threshold=2.0,
    ...
    # Plot all points (display in decimal units like the paper: 0.30, 0.60, ...)
    if sft_nt:
        plt.scatter(
            [x / 100.0 for x in sft_nt],
            [y / 100.0 for y in sft_pt],
            label='SFT',
            alpha=0.5,
            s=80,
            color='blue',
    ...
    if rl_nt:
        plt.scatter(
            [x / 100.0 for x in rl_nt],
            [y / 100.0 for y in rl_pt],
            label='RL (GRPO)',
            alpha=0.5,
            s=80,
            color='red',
    ...
    if use_curve_fit:
        ...
        if len(sft_near_x) >= 3:
            ...
            plt.plot(
                    [x / 100.0 for x in sft_curve_x],
                    [y / 100.0 for y in sft_curve_y],
                    '--', color='blue', 
        ...
        if len(rl_near_x) >= 3:
            ...
            plt.plot(
                    [x / 100.0 for x in rl_curve_x],
                    [y / 100.0 for y in rl_curve_y],
                    '--', color='red', 
        ...
        if len(sft_pareto_x) > 1:
            plt.plot(
                    [x / 100.0 for x in sft_pareto_x],
                    [y / 100.0 for y in sft_pareto_y],
                    'o--', color='orange', 
        ...
        if len(rl_pareto_x) > 1:
            plt.plot(
                    [x / 100.0 for x in rl_pareto_x],
                    [y / 100.0 for y in rl_pareto_y],
                    's--', color='blue', 
    ...
    # Labels and title
    # Keep axis range tied to observed points so the figure doesn't visually
    # extend far left when there is no data there.
    if all_nt:
        nt_min = min(all_nt)
        nt_max = max(all_nt)
        x_pad = max(0.5, (nt_max - nt_min) * 0.03)
        plt.xlim((nt_min - x_pad) / 100.0, (nt_max + x_pad) / 100.0)

    ax = plt.gca()
    # Match paper style: show NT ticks every 0.10 (10 percentage points) with decimals.
